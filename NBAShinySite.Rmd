...
shot_map <- nba_pbp |>
  ...
  filter(player_n %in% season25$player | player_n %in% c("Bogdan Bogdanovic", "Luka Doncic", "Nikola Jokic", "Alperen Sengun", "Dennis Schroder", "Nikola Vucevic", "Nikola Jović")) |>
...
```{r echo = FALSE}
inputPanel(
  selectInput("distg", label = "Number of Players",
...
renderTable({
  mean_shot |>
    ...
    slice_head(n = as.numeric(input$distg))
})
```
...
geom_point(data = playd, aes(x = coordinate_y, y = -coordinate_x, color = result)) +
